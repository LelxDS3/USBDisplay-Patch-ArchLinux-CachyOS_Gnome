...
#include <linux/scatterlist.h>		/* struct sg_table */
...
static __maybe_unused void usb_hal_image_to_yuv(struct usb_hal_dev* usb_dev)
{
    int row, i;
...
static __maybe_unused void usb_hal_package_block(struct usb_hal_dev* usb_dev)
{
...
static __maybe_unused void usb_hal_update_change_rects(struct usb_hal_dev* usb_dev)
{
	int row, col;
	int bytesperline;
...
static __maybe_unused void usb_hal_clear_rect(struct usb_hal_dev* usb_dev, int index)
{
...
/* FRAME mode: convert the full XRGB8888 desktop image to RGB888 (B,G,R order,
 * dropping the X byte) into usb_buf with no block header. Mirrors the proven
 * fbdev transport for this MS912C/DIGITAL panel. */
static void usb_hal_image_to_rgb888(struct usb_hal_dev* usb_dev)
{
	int total = usb_dev->mode.width * usb_dev->mode.height;
	u8 *src = usb_dev->desktop_buf.buf;	/* XRGB8888 in memory: B,G,R,X */
	u8 *dst = usb_dev->usb_buf.buf;		/* FRAME mode: raw RGB888, no header */
	int i;
...
static int usb_hal_dev_send_frame(struct usb_hal_dev* usb_dev, struct urb* data_urb, unsigned char* zero_msg, int ep)
{
	...
	if (USB_HAL_TRANS_MODE_MANUAL_BLOCK == usb_dev->trans_mode) {
		/* Block mode (MSWindows transport): only the dirty region changed
		 * since the last frame is sent, as YUV422, wrapped in a block header.
		 * Skip the transfer entirely when nothing changed. */
		...
		/* FRAME mode: send the whole frame as raw RGB888 (no block header),
		 * mirroring the proven fbdev transport. */
		usb_hal_image_to_rgb888(usb_dev);
	}
	mutex_unlock(&usb_dev->usb_buf.mutex);
	...
	if (USB_HAL_TRANS_MODE_MANUAL_BLOCK != usb_dev->trans_mode) {
		/* FRAME mode: trigger the buffer swap so the chip scans out the new frame
		 * (mirrors the proven fbdev transport). Block mode needs no trigger. */
