 ...
 * 0 (default) = FRAME mode + full-frame RGB888: the transport proven on this
 *               MS912C/DIGITAL panel by the fbdev driver (color 0x11/0x00).
 * 1           = MANUAL_BLOCK + dirty-rects + YUV422: the efficient transport
 *               used by the Windows driver (only changed regions, 2B/px).
 * Needs a reboot (module is loaded at boot / not unloadable while in use).
 */
static bool block_transfer;
module_param(block_transfer, bool, 0600);
MODULE_PARM_DESC(block_transfer, "use MANUAL_BLOCK+YUV422 dirty-rect transport (1) vs full-frame FRAME RGB888 (0, default)");
...
/* We transmit full-frame RGB888 in FRAME mode (see usb_hal_thread.c), so
     * signal RGB packing: this yields color_in=0x11, color_out=0x00 for RGB
     * formats -- the exact values proven working on this panel via the fbdev. */
    if (block_transfer) {
        /* Official/MSWindows transport: YUV422 block mode. */
        ...
        usb_dev->color_out = desc->color_fmt;
...
int usb_hal_update_frame(struct usb_hal* hal, u8* buf, int pitch, u32 len, u32 fourcc, int try_lock)
{
    ...
    /* Always capture the frame into desktop_buf even if the pipe isn't ENABLED
     * yet: the DRM commit delivers the first frame (plane update) BEFORE the
     * crtc enable, so dropping it here lost the first frame. The state machine
     * still gates the actual USB send on ENABLED (it drops UPDATE events when
     * not enabled), and the crtc enable re-delivers the current fb. */
...
struct usb_hal* usb_hal_init(struct usb_interface *interface, const struct usb_device_id *id, struct kfifo* fifo, u32 index)
{
    ...
    usb_dev->fifo = fifo;
    usb_dev->index = index;
    usb_dev->vpack_out = USB_HAL_COLOR_FORMAT_RGB888;
    usb_dev->vpack_in  = USB_HAL_COLOR_FORMAT_RGB888;
    /* FRAME mode + full-frame RGB888 + trigger is the transport just proven on
     * this exact MS912C/DIGITAL panel by the fbdev driver (color 0x11/0x00,
     * vic 150). */
    usb_dev->trans_mode = USB_HAL_TRANS_MODE_FRAME;
    if (block_transfer) {
        /* Chrome protocol / Windows driver transport: block mode over YUV422. */
        usb_dev->vpack_out = USB_HAL_COLOR_FORMAT_YUV422;
        usb_dev->vpack_in  = USB_HAL_COLOR_FORMAT_YUV422;
        usb_dev->trans_mode = USB_HAL_TRANS_MODE_MANUAL_BLOCK;
        dev_info(&udev->dev, "block_transfer=1: using MANUAL_BLOCK + YUV422 dirty-rect transport\n");
    }
