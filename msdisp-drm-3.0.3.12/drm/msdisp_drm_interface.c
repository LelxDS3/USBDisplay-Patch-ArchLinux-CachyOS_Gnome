...
int msdisp_drm_register_usb_hal(struct drm_device* drm, int pipeline_index, struct msdisp_usb_hal* usb_hal)
{
    ...
    /* The output just became connected. Notify userspace and the in-kernel
     * fbdev client so the console (fbcon) comes up on the panel and any
     * compositor repaints -- this is also what restores the image after a
     * replug (the USB transport re-attaches through here). */
